...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        Console.WriteLine($"\nWysyłam zbiorcze zapytanie do LLM dla {universityData.Instructors.Count} prowadzących...");

        // Jedno zapytanie dla wszystkich
        ...
        // Zapisanie gotowych wyników do nowego pliku JSON
        string outputFilePath = "wynik_preferencje.json";
        string finalJson = JsonSerializer.Serialize(universityData.Instructors, new JsonSerializerOptions
        {
            WriteIndented = true
        });

        File.WriteAllText(outputFilePath, finalJson);
        Console.WriteLine($"\nGotowe preferencje zapisano do pliku: {outputFilePath}");
    }
}